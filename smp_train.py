...
resize_dims = (480, 480) # must be % 32 = 0
t_train = A.Compose([A.Resize(height=resize_dims[0], width=resize_dims[1], interpolation=cv2.INTER_NEAREST),
    A.HorizontalFlip(),
    A.VerticalFlip(),
...
t_val = A.Compose([A.Resize(resize_dims[0], resize_dims[1], interpolation=cv2.INTER_NEAREST), 
    A.HorizontalFlip(),
    A.GridDistortion(p=0.2)])
...
def fit(epochs, model, train_loader, val_loader, criterion, optimizer, scheduler, patch=False):
    ...
    # with open(os.path.join(f"./results/{model_name}/train_progress.txt"), "w") as f:
    #     f.write(f"Train Progress of {model_name} model \n")
    ...
    for e in range(epochs):
        ...
        for i, data in enumerate(tqdm(train_loader)):
            ...
            if min_loss > (test_loss / len(val_loader)):
                ...
                if decrease % 5 == 0:
                    print('saving model...')
                    torch.save(deepcopy(de_parallel(model)),
                               f'./results/{model_name}/{model_name}-_mIoU-{val_iou_score / len(val_loader):.3f}.pt')
...
if TRAIN:
    for _, model in models.items():
        ...
        print(model_name+"...")
        if LOAD_pt:
            model = torch.load(f'./results/{model_name}/{model_name}_{TRY_num - 1}.pt')
        ...
        torch.save(deepcopy(de_parallel(model)), f'./results/{model_name}/{model_name}_{TRY_num}.pt')


        with open(f'./results/{model_name}/{model_name}_history_{TRY_num}.json', 'w') as f:
